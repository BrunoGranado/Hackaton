...
void insere_MarketPlace(MARKETPLACE *vetMkp, PRODUTO *estoque);
...
void finalizar(FILE* erp, FILE* mk1, FILE* mk2, MARKETPLACE *vetMkp, PRODUTO *estoque);

int main(void) {
	
	const int tamEstoque = 5, tamSistema = 2;
	int i, j, escolha;
	...
	do{
		...
		switch(escolha){
			...
			case 2:
				printf("---------------------------\n");
				insere_MarketPlace(sistema, estoque);
				printf("---------------------------\n");
				break;
			case 3:
				printf("---------------------------\n");
...
void insere_MarketPlace(MARKETPLACE *vetMkp, PRODUTO *estoque){
	...
	printf("Em qual marketplace?(1 ou 2, para inserir em todos escolha 0)");
	scanf("%d", &escolha);
	if(escolha==0){
		for(i=0;i<2;i++){
			livre=false;
	...
	/*for(i=0;i<2;i++){
		for(j=0;j<10;j++){
			if(vetMkp[i].anuncios[j].status==true){
				printf("verdadeiro\n");
			}
			printf("Codigo:%d\n", vetMkp[i].anuncios[j].code);
			printf("Nome:%s\n", vetMkp[i].anuncios[j].nome);
			printf("Valor:%f\n\n", vetMkp[i].anuncios[j].preco);
		}
	}*/
	return;	
}
...
void remove_anuncio(MARKETPLACE *sistema, int tam){
	...
	printf("insira o codigo do anuncio a ser removido:");
	scanf("%d", &aux);
	
	for(i=0; i<2; i++) {
		for(j=0; j<10; j++) {
			if(sistema[i].anuncios[j].code == aux) {
				printf("Anuncio removido com sucesso");
				sistema[i].anuncios[j].status = false;
				break;
			}
		}
